%%
function PathGen(dir_name, ini_pva, mot_def, out_typ, sim_mode, sen_conf, imu_conf, vib_param)
debg=0;
...
if (nimu>0)
    ...
    if (~isempty(vib_param))
        for in=1:nimu
            ...
            vib_state(:,in)=randn(3,1)*vib_param(in,3);     %initial vibration angles
...
if (debg)
    ...
    deb_vel=zeros(3,10000);
    deb_pos=zeros(3,10000);
    deb_att=zeros(3,10000);
    deb_ctr=0;
end
...
%         vel_b=Cbn'*vel_n;
        ...
        if (mod(sim_cnt,intmul)==0)
            ...
            if (sim_mode==0)
                vr_a=[(sim_cnt/intmul);pos_n+posdn;vel_b;att_n];
            else
...
%                 deb_att(:,deb_ctr)=dcm2euler_v000(strap_sCsn(:,:,1)*sCsn');
                ...
                deb_pos(:,deb_ctr)=strap_pos_n-pos_dn;
                ...
                switch out_typ(sr_a,1)
                    case {1}
                        if (mod(sim_cnt,out_typ(sr_a,2))==0)
                            if (sim_mode==0)
                                vr_a=[(sim_cnt/intmul);vel_n;pos_n];
                            elseif(sim_mode==1)
                    ...
                    case {2}
                        if (mod(sim_cnt,out_typ(sr_a,2))==0)
                            fwrite(fodo, [(sim_cnt/intmul);odo_vel;odo_pos],'double');
                            odo_pos=0;
                        end
                end
            end
...
if (styp==0)
    w_en_n=zeros(3,1);
    w_en_n(1)=vel_n(2)/(Re+h);
    w_en_n(2)=-vel_n(1)/(Rn+h);
    w_en_n(3)=-vel_n(2)*(sL/cL)/(Re+h);
...
if (styp==0)
    pos_n_dot=zeros(3,1);
    pos_n_dot(1)=vel_n(1)/(Rn+h);
    pos_n_dot(2)=vel_n(2)/(Re+h)/cL;
    pos_n_dot(3)=-vel_n(3);
elseif (styp==1)
    pos_n_dot=zeros(3,1);
    pos_n_dot(1)=vel_n(1);
...
for in=1:nimu
    ...
    vib_state_new(:,in)=vib_model(1,in)*vib_state(:,in)+vib_model(2,in)*randn(3,1);
