...
% also tied to this epoch t0, therefore, they must be rigidly fixed
% together and start from stationary with ZUPT correction at least for 1min
...
classdef EKF_filter_s0framelet < handle
    ...
    properties (SetAccess = private)
        ...
        rvqs2e; % position of IMU in e frame, v in e, and qs2e, not used
    ...
    methods
        function filter = EKF_filter_s0framelet(options)
            ...
            qs2e=quatmult_v001(rotro2qr(Ce2n0), initqbn,1); % assume body frame is sensor frame
            
            filter.rqs02e=[ecef2geo_v000(options.inillh_ant,1); qs2e];
            ...
            filter.camPose(5:7)=options.Cimu2cam*(-options.Tcam2body); % imu is the body frame
            ...
            smallValue=1e-3;
            filter.p_k_k(1:3,1:3)=eye(3)*smallValue^2; %position errors in meter
            filter.p_k_k(4:6,4:6)=eye(3)*smallValue^2; %vel error in m/s
            filter.p_k_k(filter.imuOrientSIP+(0:2),filter.imuOrientSIP+(0:2))=...
                diag([smallValue,smallValue,smallValue*3].^2);
            filter.p_k_k(10:12,10:12)=diag([1;1;1]*0.01^2); % m/s^2 of gravity in s0 frame
            ...
            filter.p_k_k(filter.imuScaleFactorSIP+(0:2),filter.imuScaleFactorSIP+(0:2))=zeros(3); %4*eye(3)*IMU_ERRDEF.acc_scale_var;
            filter.p_k_k(filter.imuScaleFactorSIP+(3:5),filter.imuScaleFactorSIP+(3:5))=zeros(3); % 4*eye(3)*IMU_ERRDEF.gyro_scale_var;
